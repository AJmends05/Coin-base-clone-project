...
import { CRYPTO_LIST, STOCKS_LIST, PREDICTIONS_LIST, PERPETUALS_LIST, FUTURES_LIST, SEARCH_TABS } from "../../data/constants";

const ALL_DATA = {
    Crypto: CRYPTO_LIST,
    Stocks: STOCKS_LIST,
    Predictions: PREDICTIONS_LIST,
    Perpetuals: PERPETUALS_LIST,
        ...
        <div className="sr-row">
            <div className="sr-row-left">
                <div className="sr-coin-icon" style={{ background: item.iconBg }}>
                    <i className={`bi ${item.icon}`} style={{ color: "white" }} />
                </div>
                <div className="sr-row-info">
                    <div className="sr-row-name">
                        {item.name}
                        {showRank && item.rank && <span className="sr-rank-badge">#{item.rank}</span>}
                    </div>
            ...
            <div className="sr-row-right">
                ...
                <div className={`sr-row-chg ${item.up ? "up" : "dn"}`}>
                    {item.chg === 0 ? "0.00%" : `${item.up ? "↗" : "↘"} ${Math.abs(item.chg).toFixed(2)}%`}
                </div>
            </div>
        </div>
    );
...
function getTopResults() {
    return {
        CRYPTO: CRYPTO_LIST.slice(0, 3),
        STOCKS: STOCKS_LIST.slice(0, 3),
    };
}
        ...
        <div className="sr-backdrop" ref={overlayRef} onClick={handleBackdropClick}>
            <div className="sr-modal" role="dialog" aria-label="Search">
                <div className="sr-input-wrap">
                    ...
                    <input ref={inputRef} className="sr-input" placeholder="Search assets" value={query} onChange={(e) => setQuery(e.target.value)} />
                    {query && (
                ...
                {!hasQuery && (
                    <div className="sr-tabs">
                        {SEARCH_TABS.map((tab) => (
                            <button key={tab} className={`sr-tab${activeTab === tab ? " active" : ""}`} onClick={() => setActiveTab(tab)}>
                                {tab}
                            </button>
                        ))}
                    </div>
